impl Solution {
    fn binary_search(lookup: &Vec<i32>, n: i32) -> i32 {
        let mut low = 0;
        let mut high = lookup.len();
...
mod tests {
    ...
    #[test]
    fn it_works() {
        let scenarios = vec![(1, 22), (1000, 1333), (3000, 3133)];

        scenarios
            .into_iter()
            .enumerate()
            .for_each(|(idx, (input, expected))| {
                let result = Solution::next_beautiful_number(input);
